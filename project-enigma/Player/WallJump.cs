...
public partial class WallJump : State
{
    ...
    //Returns 0: Not Colliding, 1: Left, 2: Right
    private int LeftOrRight()
    {
        bool right = GetNode<RayCast2D>("WallCheck/RightCheck").IsColliding();
        bool left = GetNode<RayCast2D>("WallCheck/Leftheck").IsColliding();
        if (left)
        {
            return 1;
        }
    ...
    public override void Enter()
    {
        ...
        int side = LeftOrRight();
        if (side == 0)
        {
            GD.Print("Does not collide");
        }

        const float AngleChange = (float)Math.PI / 4;
        if (side == 1)
        {
